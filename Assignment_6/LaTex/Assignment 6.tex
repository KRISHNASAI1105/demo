...
\begin{lstlisting}
https://github.com/KRISHNASAI1105/demo/blob/main/Assignment_6/LaTex/Assignment%206.tex
\end{lstlisting}

\subsection*{\boldsymbol{Problem\ number\ CSIR\ UGC\ NET\ Dec\ 2014\ Q.104}}
...
\end{align}
\begin{align}
  \pr{X_2 > X_1} &= \int\limits_{-\infty}^\infty f_X(x) \int\limits_{-\infty}^x                  f_X(t)\ d t d x \\
                 &= \int\limits_{-\infty}^\infty f_X(x) F_X(x) d x \\
                 &= \cfrac{F^{2}_X(x)}{2} \biggr \vert_{-\infty}^{\infty} \\
                 &= \dfrac{1}{2}.
\end{align}
\begin{multline}
    \pr{X_4 > Max(X_1,X_2) > X_3} =\int\limits_{-\infty}^\infty f_X(x)\int\limits_{-\infty}^x f_X(t).\\\comb{2}{1} \sbrak{\int\limits_{-\infty}^t f_X(w) d  w} \int\limits_{-\infty}^t f_X(z)\ d z d t d    x        
\end{multline}
\begin{align}
...
&= \dfrac{2}{3}\cfrac{F^{4}X(x)}{4} \biggr \vert_{-\infty}^{\infty} \\    
&= \dfrac{1}{6}
\end{align}
\begin{align}
\pr{X_4 > X_3 > Max(X_1,X_2)} &= \nonumber \int\limits_{-\infty}^\infty f_X(x) 
                                  \int\limits_{-\infty}^x f_X(t) 
                                  \int\limits_{-\infty}^t f_X(z). \\
                               &  \comb{2}{1} \sbrak{\int\limits_{-\infty}^t       f_X(w) d w}\ d z d t d x \\
                               &= \nonumber \int\limits_{-\infty}^\infty f_X(x) 
                                  \int\limits_{-\infty}^x f_X(t) \\
